/**
 * PPTX解析核心工具
 * 提取自 useImport.ts，用于多个模块复用
 * 
 * 使用场景：
 * 1. 编辑器PPTX上传（useImport.ts）
 * 2. Docs创建文档（Docs/index.vue）
 * 3. Sales回传功能（Sales/Profile.vue）
 */
 ...
 * 解析线条元素
 */
const parseLineElement = (el: Shape, ratio: number, themeColor: string) => {
 ...
 * @param file - PPTX文件对象
 * @param options - 解析选项
 * @returns 解析后的slides、theme和viewport信息
 */
export async function parsePPTXToSlides(
  file: File,
  options?: {
    fixedViewport?: boolean  // 是否固定viewport为1000
    defaultTheme?: any       // 默认主题配置
  }
): Promise<{
  slides: Slide[]           // 解析后的幻灯片数组
  theme: any                // 主题配置
  viewportSize?: number     // viewport大小（如果不固定）
}> {
  return new Promise((resolve, reject) => {
    ...
    const { fixedViewport, defaultTheme } = { ...defaultOptions, ...options }
    ...
    reader.onload = async e => {
      ...
      let viewportSize: number | undefined = undefined
      if (fixedViewport) {
        ratio = 1000 / width
      } else {
          ...
          for (const el of sortedElements) {
            ...
            if (el.type === 'text') {
              ...
              if (el.shapType === 'line' || /Connector/.test(el.shapType)) {
                const lineElement = parseLineElement(el, ratio, theme.themeColors[0])
